...
(async () => {
    ...
    try {
        ...
        const name = "نوري فيصل";
        console.log(`Starting targeted remote scraper debug for: ${name}`);
        ...
        const scriptContent = `
            ...
            async function run() {
                try {
                    ...
                    console.log('Total Reports Found for ${name}:', reports.length);
                    reports.forEach((r, i) => {
                        console.log((i+1) + '. Date:', r.date, '| Name:', r.name);
                    });
                } catch(e) {
