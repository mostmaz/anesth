                ...
                <table className="w-full text-[10px] text-left border-collapse table-fixed">
                    <thead>
                        <tr className="bg-slate-100 print:bg-slate-100">
                            <th className="border p-1 w-1/4">Date & Time</th>
                            <th className="border p-1 text-center text-red-700 w-1/6">HR (bpm)</th>
                            <th className="border p-1 text-center text-blue-700 w-1/4">BP (mmHg)</th>
                            <th className="border p-1 text-center text-emerald-700 w-1/6">SpO2 (%)</th>
                            <th className="border p-1 text-center text-amber-700 w-1/6">Temp (°C)</th>
                            <th className="border p-1 text-center text-purple-700 w-1/6">RBS</th>
                        </tr>
                    </thead>
                    <tbody>
                        {vitals.map((v, i) => (
                            <tr key={v.id} className={`${i % 2 === 0 ? 'bg-white' : 'bg-slate-50'} print:break-inside-avoid print:bg-white`}>
                                <td className="border p-1 font-medium break-words">
                                    {new Date(v.timestamp).toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                                </td>
                                <td className="border p-1 text-center font-bold">{v.heartRate || '--'}</td>
                                <td className="border p-1 text-center">{v.bpSys}/{v.bpDia}</td>
                                <td className="border p-1 text-center">{v.spo2}%</td>
                                <td className="border p-1 text-center">{v.temp || '--'}</td>
                                <td className="border p-1 text-center">{v.rbs || '--'}</td>
                            </tr>
                        ))}
                        {vitals.length === 0 && (
                            <tr>
                                <td colSpan={6} className="border p-4 text-center text-slate-400 italic">No vitals recorded in this period</td>
                            </tr>
                        )}
                    </tbody>
                </table>
